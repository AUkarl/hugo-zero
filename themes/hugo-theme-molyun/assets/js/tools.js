    ...
    document.querySelectorAll('.tool-sub-item').forEach(function (item) {
      item.addEventListener('click', function () {
        ...
        if (sub === '格式转换' || sub === '音频转换' || sub === '图片压缩' || sub === '代码格式化') {
          ...
          html = '<input type="text" placeholder="' + t('tool-input-host') + '" value="molyun.com" /><button onclick="alert(\'' + t('tool-querying') + '\')">' + t('tool-query') + '</button><span class="hint">' + t('tool-demo') + '</span>';
